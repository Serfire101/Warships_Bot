...
public class Warship {
    ...
    public int setShips (CallbackQuery update_button){
        ...
        if(field_my[x][y] == 0){            //если клетка пуста
            for(int i = -1; i < 2; i++){
                for(int j = -1 ; j < 2; j++){   //Обхожу клетку по кругу с радиусом в 1 клетку
                    if(i!= 0 || j!= 0){          // Избегаю проверки самой клетки
                        if(  field_my[x+i][y+j] == 2) {  // Если где-то в радиусе есть кораблик
                            if(i == 0 ||j == 0){       //Проверяю, находится ли корабль на углу или сбоку/сверху(false - на углу)
                                if(i == 0){             //Проверяю по х или у идти дальше, чтобі посмотреть сколько уже палуб у корабля
